...
public final class Scheduler
    ...
    public static final String DAEMON_ID =
        getConfigurationProperty("sched.daemonId", "*"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private String readClassName(INIFileManager ifm, String taskName) {
        ...
        if (taskClassName == null) {
            throw new IllegalArgumentException(
                getMessage("SCHED_ERR_KEY_CLASS_NOT_FOUND", taskName)); //$NON-NLS-1$
        }
    ...
    private String readDescription(INIFileManager ifm, String taskName) {
        ...
        if (taskDescription == null) {
            throw new IllegalArgumentException(
                getMessage("SCHED_ERR_KEY_DESCRIPTION_NOT_FOUND", taskName)); //$NON-NLS-1$
        }
    ...
    private void checkRescheduleTask(Calendar now, SchedulerTask task) {
        ...
        if (task.getTaskNextStartTime() == null || task.getTaskNextStopTime() == null) {
            ...
            LOG.info(getMessage("SCHED_LOG_TASK_SCHEDULED", //$NON-NLS-1$
                new String[] {
                    task.getTaskName(),
    ...
    private void checkDaemonStart(SchedulerTask task) {
        ...
        if (!task.isStarting() && !task.isExecuting() && !task.isDaemonExecuted()) {
            LOG.info(getMessage("SCHED_LOG_DAEMON_SCHEDULED", task.getTaskName())); //$NON-NLS-1$
    ...
    public void scheduleTask(String taskName, Class<?> taskClass, String taskDescription,
        ...
        synchronized (tasks) {
            ...
            try {
                ...
                throw new IllegalArgumentException(
                    getMessage("SCHED_ERR_TASK_CLASS_INVALID", taskClass.getName(), nsme.toString()), nsme); //$NON-NLS-1$
            } catch (InstantiationException ie) {
                throw new IllegalArgumentException(
                    getMessage("SCHED_ERR_TASK_CLASS_INVALID", taskClass.getName(), ie.toString()), ie); //$NON-NLS-1$
            } catch (IllegalAccessException iae) {
                throw new IllegalArgumentException(
                    getMessage("SCHED_ERR_TASK_CLASS_INVALID", taskClass.getName(), iae.toString()), iae); //$NON-NLS-1$
